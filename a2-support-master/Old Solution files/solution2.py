...
class Solver:
    ...
    def pi_initialise(self):
        ...
        # In order to ensure compatibility with tester, you should avoid adding additional arguments to this function.
        #
        self.pi_states = [self.environment.get_init_state()]
        frontier = deque()
        frontier.append(self.environment.get_init_state())
            ...
            for x in ROBOT_ACTIONS:
                ...
                for m in movements:
                    ...
                    if (cost < ACTION_BASE_COST[m] + ACTION_PUSH_COST[m]):
                        valid, cost, next_state = self.environment.get_iterations(current, m)
                        if valid:
                            if next_state not in self.pi_states and nxt_state == next_state:
                                    frontier.append(next_state)
                                    if self.environment.is_solved(next_state):
                                        self.exit_state_pi.append(next_state)
                                    self.pi_states.append(next_state)
                            
               
        self.state_values = {state: 0 for state in self.pi_states}
        self.policy_pi = {state: FORWARD for state in self.pi_states}
        #print((self.policy_pi.values()))
    ...
    def pi_iteration(self):
        ...
        state_numbers = np.array(range(len(self.pi_states)))  # indices of every state
        state_actions = np.array(range(len(ROBOT_ACTIONS))) #Action indices
        t_pi = self.t_model[state_numbers, self.la_policy]
        r_pi = self.r_model[state_numbers, self.la_policy]
        values = np.linalg.solve(np.identity(len(self.pi_states)) - (self.gamma * t_pi), self.r_model)
        policies = np.linalg.solve((np.identity(len(self.pi_states)) - (self.gamma * t_pi)), r_pi)
        
        self.state_values = {s: policies[i] for i, s in enumerate(self.pi_states)}
        # new_policy = {s: ROBOT_ACTIONS[self.la_policy[i]] for i, s in enumerate(self.pi_states)}
        #print(self.state_values.values())

        new_policy = self.policy_improvement()
        
        #print(new_policy.values())
        ...
        # for x in ROBOT_ACTIONS:
        #     r_pi = self.r_model[state_nums,:]
        # t_pi = self.t_model[state_nums, self.la_policy]
    ...
    def pi_select_action(self, state: State):
        ...
        for x in ROBOT_ACTIONS:
            ...
            for stoch_action, p in self.environment.stoch_action2(x):
                ...
                v = p * (self.get_reward(state) + (self.gamma * self.state_values[next_state]))
                vi = vi + v
                total += v
                action_values[x] = total
