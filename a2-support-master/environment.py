...
class Environment:
    ...
    def stoch_action(self, action):
        """
        Returns probabilites with which each ation will actually occur given the requested action.
        :param action: action requested by the agent
        :return: (Probability distribution over actual actions that may occur, actions occuring over distribution)
        """
        
        p_action_only = 1 - self.drift_cw_probs[action] - self.drift_ccw_probs[action] - (self.drift_cw_probs[action] * self.double_move_probs[action]) - (self.drift_ccw_probs[action] * self.double_move_probs[action])
        if action == SPIN_LEFT:
            return {SPIN_LEFT: [p_action_only, p_action_only * self.double_move_probs[action]]}

        elif action == SPIN_RIGHT:
            return {SPIN_RIGHT: [p_action_only, p_action_only * self.double_move_probs[action]]}
        
        elif action == FORWARD:
            return {FORWARD: [p_action_only, (p_action_only)*self.double_move_probs[action]],
                    SPIN_LEFT: [self.drift_ccw_probs[action], (self.drift_ccw_probs[action])*self.double_move_probs[action]], 
                    SPIN_RIGHT: [self.drift_cw_probs[action], (self.drift_cw_probs[action])*self.double_move_probs[action]]}
        
        elif action == REVERSE:
            return {REVERSE: [p_action_only, (p_action_only)*self.double_move_probs[action]], 
                    SPIN_LEFT: [self.drift_ccw_probs[action], (self.drift_ccw_probs[action])*self.double_move_probs[action]], 
                    SPIN_RIGHT: [self.drift_cw_probs[action], (self.drift_cw_probs[action])*self.double_move_probs[action]]}  
        
    def stoch_action2(self, action):
        """
        Returns probabilites with which each ation will actually occur given the requested action.
        :param action: action requested by the agent
        :return: (Probability distribution over actual actions that may occur, actions occuring over distribution)
        """
        #action lists
        # [([action], p_action_only), ([SLANT_LEFT, action]), ([SPIN_RIGHT, action]), ([SPIN_LEFT, action, action]), ([SPIN_RIGHT, action, action]), ([action, action])]

        p_action_only = 1 - self.drift_cw_probs[action] - self.drift_ccw_probs[action] - (self.drift_cw_probs[action] * self.double_move_probs[action]) - (self.drift_ccw_probs[action] * self.double_move_probs[action])
        p_drift_cw_action = self.drift_cw_probs[action]
        p_drift_ccw_action = self.drift_ccw_probs[action]
        p_drift_cw_action_2 = (self.drift_cw_probs[action] * self.double_move_probs[action])
        p_drift_ccw_action_2 = (self.drift_ccw_probs[action] * self.double_move_probs[action])
        p_action_2 = p_action_only * self.double_move_probs[action]
       

        return [([action], p_action_only), ([SPIN_LEFT, action], p_drift_ccw_action),
                ([SPIN_RIGHT, action], p_drift_cw_action), ([SPIN_LEFT, action, action], p_drift_ccw_action_2),
                ([SPIN_RIGHT, action, action], p_drift_cw_action_2), ([action, action], p_action_2)]
        # action_p = dict()

        # action_p[0] = 1 - self.drift_cw_probs[action] - self.drift_ccw_probs[action] - self.double_move_probs[action] - (self.drift_cw_probs[action] * self.double_move_probs[action]) - (self.drift_ccw_probs[action] * self.double_move_probs[action])
        # action_p[1] = self.drift_cw_probs[action]
        # action_p[2] = self.drift_ccw_probs[action]
        # action_p[3] = self.double_move_probs[action]
        # action_p[4] = self.drift_cw_probs[action] * self.double_move_probs[action] 
        # action_p[5] = self.drift_ccw_probs[action] * self.double_move_probs[action]
     
        # return action_p

        # return [1-self.drift_cw_probs[action]-self.drift_ccw_probs[action], 
        #         self.drift_cw_probs[action], self.drift_ccw_probs[action], 
        #         self.double_move_probs[action], self.drift_cw_probs[action]*self.double_move_probs[action], 
        #         self.drift_ccw_probs[action]*self.double_move_probs[action]]

    def stoch_action_list(self, action):

        action_cost = dict()
        action_cost[0] = [action]
        action_cost[1] = [SPIN_CW, action]
        action_cost[2] = [SPIN_CCW, action]
        action_cost[3] = [action, action]
        action_cost[4] = [SPIN_CW, action, action]
        action_cost[5] = [SPIN_CCW, action, action]
    ...
    def get_transition_probabilities(self, s, a):
        ...
        probabilities = {}
        for a, p in self.stoch_action(a).items():
            cost, next_state = self.apply_action_noise()

    def get_iterations(self, state, action):
        """
        Perform the given action on the given state, and return whether the action was successful (i.e. valid and
        collision free), the cost of performing the action, and the resulting new state.
        :param state: 
