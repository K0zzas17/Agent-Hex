...
class Environment:
    ...
    def get_solved_state(self):
        ...
        """
        # I want to find where the targets are. I assume tgt_list is an x,y tupe of widget locations.
        # next I need to know where the widgets are and what kind of 
        # loop over each target
        
        # frontier = PriorityQueue()
        # frontier.put(self.get_init_state())
        # visited_hex = {self.get_init_state().robot_posit: 0} # a dict of `vertex: cost_so_far`
        # path = {self.get_init_state(): []}

        # while not frontier.empty():
        #     current = frontier.get()
        #     visited_hex.update({current: current.priority})
        ...
        tgt_wid_centres = []
        wid_positions = self.widget_init_posits
        for p in wid_positions:
            tgt_wid_centres.append(p)
        
        if self.widget_types == WIDGET3:
            for t, w in zip(self.target_list, tgt_wid_centres):
                if t[0] > 0 and t[1] > 0 and ([0], t[1]) not in self.obstacle_map:
                    self.widget_init_orients
                    tgt_wid_centres[tgt_wid_centres.index(w)] = t

            
        #self.solved_state = start
        tgt_wid_centres = tuple(tgt_wid_centres)

        # for x in self.widget_types:
        #     if x == WIDGET3:
        #         for t, w in zip(self.target_list, tgt_wid_centres):
        #             tgt_wid_centres[tgt_wid_centres.index(w)] = t
        #             if w(t[0], t[1]) in self.target_list and w(t[0], t[1]-1) not in self.target_list:
        #                 tgt_wid_centres[tgt_wid_centres.index(w)] = (t[0])
        #     elif x == WIDGET4:
        #         for t, w in zip(self.target_list, tgt_wid_centres):
        #             tgt_wid_centres[tgt_wid_centres.index(w)] = t
        #     elif x == WIDGET5:
        ...
        return State(self, self.robot_init_posit, self.robot_init_orient, 
            tgt_wid_centres, self.widget_init_orients, self.force_valid)


    def manhattan_distance_heurestic(self, state: State):
